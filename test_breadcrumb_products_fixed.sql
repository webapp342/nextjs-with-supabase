-- ====================================
-- BREADCRUMB SİSTEMİ TEST VERİLERİ (DÜZELTİLMİŞ)
-- ====================================

-- İlk olarak database güncellemesinin yapıldığından emin olun
-- database_breadcrumb_fixed.sql dosyasını çalıştırmış olmanız gerekiyor

-- Önce mevcut bir user ID'si bulalım veya test user oluşturalım
DO $$
DECLARE
    test_user_id UUID;
BEGIN
    -- Mevcut herhangi bir user ID'sini bul
    SELECT id INTO test_user_id FROM auth.users LIMIT 1;
    
    -- Eğer hiç user yoksa, geçici test user'ı oluştur
    ...
    -- Test user ID'sini geçici tabloda sakla
    CREATE TEMP TABLE temp_user_id AS SELECT test_user_id as user_id;
END $$;
...
-- 4. Test ürünlerini ekle (mevcut user ID ile)
...
SELECT 
  ...
  true,
  tu.user_id
FROM categories_new c, brands b, product_types pt, temp_user_id tu
...
-- Kullanılan user ID'sini göster  
SELECT 'Kullanılan user ID: ' || user_id as info FROM temp_user_id;
