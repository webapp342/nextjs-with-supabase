...
export function EnhancedBreadcrumb({ 
        ...
        if (pathSegments.includes('product-details')) {
          ...
          if (brand) {
            ...
            if (categorySlug) {
              ...
              if (category) {
                brandItems.push({
                  label: `${category.name} ${brand.name}`, // "عطر زنانه هات لاو" formatı
  ...
  const renderBreadcrumbItems = (items: BreadcrumbItem[], allowAllLinks = false) => (
    <div className="flex items-center gap-2 text-sm font-lalezar" dir="rtl">
        ...
        <div key={index} className="flex items-center gap-2">
          {allowAllLinks || index !== items.length - 1 ? (
            <>
              <Link 
                href={item.href}
                className="text-gray-500 hover:text-gray-700 transition-colors font-lalezar"
              >
                {item.label}
              </Link>
  ...
  // Her ikisini de göster (temiz versiyon)
  return (
    <div className="space-y-2 py-2" dir="rtl">
